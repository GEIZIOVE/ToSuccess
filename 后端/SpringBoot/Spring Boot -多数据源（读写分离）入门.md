...
在 [「3.8 详细测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/#) 的场景二 `#method02()` 的测试，它会抛出异常。而对于本小节使用 Sharding-JDBC 的情况下，正常跑通。这是为什么呢？

原因实际在 [「2.3 方案三」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/#) 已经解释了：**分库分表中间件返回的 Connection 返回的实际是动态的 DynamicRoutingConnection ，它管理了整个请求（逻辑）过程中，使用的所有的 Connection ，而最终执行 SQL 的时候，DynamicRoutingConnection 会解析 SQL ，获得表对应的真正的 Connection 执行 SQL 操作**。
...
> 示例代码对应仓库：[lab-17-dynamic-datasource-sharding-jdbc-02](https://github.com/YunaiV/SpringBoot-Labs/tree/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02) 。
...
下面，我们来使用 Sharding-JDBC 来实现读写分离。整个的示例，我们会和 [「3. baomidou 读写分离」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/#) 是一样的功能，方便胖友做类比。

## 9.1 引入依赖

和 [「8.1 引入依赖」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/#) 一致。

## 9.2 Application

和 [「8.2 Application」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/#) 一致。

## 9.3 应用配置文件

在 [`resources`](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/main/resources) 目录下，创建 [`application.yaml`](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/main/resources/application.yaml) 配置文件。配置如下：
...
和 [「4.6 OrderMapper」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/#) 基本一致，差别是无需 `@DS` 注解，具体看如下两个传送门：

- [OrderMapper](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/main/java/cn/iocoder/springboot/lab17/dynamicdatasource/mapper/OrderMapper.java)
- [OrderMapper.xml](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/main/resources/mapper/OrderMapper.xml)

## 9.7 简单测试

创建 [OrderMapperTest](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/test/java/cn/iocoder/springboot/lab17/dynamicdatasource/mapper/OrderMapperTest.java) 测试类，我们来测试一下简单的 OrderMapper 的读写操作。代码如下：
...
- ```
  ...
  方法，测试强制访问主库。在一些业务场景下，对数据延迟敏感，所以只能强制读取主库。此时，可以使用
...
在 [`cn.iocoder.springboot.lab17.dynamicdatasource.service`](https://github.com/YunaiV/SpringBoot-Labs/tree/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/main/java/cn/iocoder/springboot/lab17/dynamicdatasource/service) 包路径下，创建 [`OrderService.java`](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/main/java/cn/iocoder/springboot/lab17/dynamicdatasource/service/OrderService.java) 类。代码如下：
...
- 我们创建了 [OrderServiceTest](https://github.com/YunaiV/SpringBoot-Labs/blob/master/lab-17/lab-17-dynamic-datasource-sharding-jdbc-02/src/test/java/cn/iocoder/springboot/lab17/dynamicdatasource/service/OrderServiceTest.java) 测试类，可以测试上面编写的两个方法。
...
然后，对于大多数场景下，方案一【基于 SpringAbstractRoutingDataSource 做拓展】，基本能够满足。这种方案，目前是比较主流的方案，大多数项目都采用。在实现上，我们可以比较容易的自己封装一套，当然也可以考虑使用 [`dynamic-datasource-spring-boot-starter`](https://mp.baomidou.com/guide/dynamic-datasource.html) 开源项目。不过呢，建议可以把它的源码撸一下，核心代码估计 1000 行左右，不要慌。
...
最后，方案三【分库分表中间件】是完美解决方案，基本满足了所有的场景。艿艿个人强烈推荐使用 [Apache ShardingSphere](https://shardingsphere.apache.org/document/current/cn/overview/) 的 [Sharding-JDBC](https://github.com/apache/incubator-shardingsphere/tree/dev/sharding-jdbc) 组件，无论胖友是有多数据源，还是分库分表，还是读写分离，都能完美的匹配。并且，Apache ShardingSphere 已经提供[多种分布式事务方案](https://shardingsphere.apache.org/document/current/cn/features/transaction/)，也能解决在文章的开头，艿艿提到的分布式事务的问题。这种类型的方案，目前很多大厂都是这样去玩的。
...
- [《芋道 Spring Boot 分库分表入门》](http://www.iocoder.cn/Spring-Boot/sharding-datasource/?self) 对应 [lab-18](https://github.com/YunaiV/SpringBoot-Labs/tree/master/lab-18) 。
- [《Spring + MyBatis 实现数据库读写分离方案》](http://www.iocoder.cn/Fight/Spring-MyBatis-implements-the-database-read-and-write-separation-scheme/?self)