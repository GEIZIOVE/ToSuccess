...
每个索引里都可以有一个或多个type，type是index中的一个逻辑数据分类，一**个type下的document，都有相同的field。**
...
<img src="img/1568632860925.png" alt="1568632860925" style="zoom:200%;" />
...
3、**token filte**r：l`owercase，stop word，synonymom`，dogs --> dog，liked --> like，Tom --> tom，a/the/an --> 干掉，mother --> mom，small --> little
...
- 往es里面直接插入数据，es会自动建立索引，同时建立对应的mapping。**(dynamic mapping)**，mapping中就自动定义了每个field的数据类型
...
- exact value和full text类型的field就决定了，在一个搜索过来的时候，对exact value field或者是full text field进行搜索的行为也是不一样的，会跟建立倒排索引的行为保持一致；
  - 比如说exact value搜索的时候，就是直接按照整个值进行匹配，
...
上边指定了analyzer是指在==索引和搜索==都使用english，如果单独想定义搜索时使用的分词器则可以通过search_analyzer属性。
...
`sql:  select * from book where name like ’ %java%’ order by price desc`
