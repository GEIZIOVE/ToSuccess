...
著作权归https://pdai.tech所有。 链接：https://pdai.tech/md/framework/orm-mybatis/mybatis-y-arch.html

## MyBatis架构概览

MyBatis框架整体设计如下:

![img](E:\Development\Typora\images\mybatis-y-arch-1.png)

### [¶](#接口层-和数据库交互的方式) 接口层-和数据库交互的方式
...
#### [¶](#使用传统的mybatis提供的api) 使用传统的MyBatis提供的API

这是传统的传递`Statement Id` 和查询参数给 `SqlSession` 对象，使用 SqlSession对象完成和数据库的交互；MyBatis 提供了非常方便和简单的API，供用户实现对数据库的增删改查数据操作，以及对数据库连接信息和MyBatis 自身配置信息的维护操作。

![img](E:\Development\Typora\images\mybatis-y-arch-2.png)
...
![img](E:\Development\Typora\images\mybatis-y-arch-4.png)
